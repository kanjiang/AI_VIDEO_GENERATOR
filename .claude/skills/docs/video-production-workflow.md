...
对应文件：`.claude/skills/seedance-2/SKILL.md`
...
- 纯白色或柔和米白色背景，不带环境、水印和无关道具
- 不做标准角色参考表，不做网格，不做蓝图式 turn-around 排版
- 用一个略微偏离中心的英雄全身视角做视觉锚点，外围再排独立的辅助研究视角
- 所有视角必须彼此分开、不重叠、不裁掉脸和肢体，并保持严格身份一致性
- 补一个小型轮廓区、表情区和细节区，方便后续图像与视频生成继续锁脸、锁轮廓、锁服装

一旦角色身份板先稳定下来，后面的故事板和视频生成就不需要反复重新猜人。

本仓库对应的模板入口：

- `.claude/skills/storyboard-table-skill/reference/character-identity-board-prompt-example.md`
...
- `.claude/skills/scene-board-skill/references/scene-board-prompt-example.md`
...
- `.claude/skills/seedance-2/references/reference-driven-video-prompt-template.md`
